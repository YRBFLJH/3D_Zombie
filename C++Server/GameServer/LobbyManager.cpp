...
void LobbyManager::RemoveClient(int id) {
    // Callers must invoke LeaveRoom(id) before RemoveClient(id)
    clients.erase(id);
    lastSeen.erase(id);
...
void LobbyManager::ProcessMessage(const uint8_t* data, size_t len,
    ...
    if (msg.has_leave_request()) {
        // Player disconnecting — must be handled at lobby level because
        // RouteToRoom skips messages when player is not in a room.
        LeaveRoom(playerId);
...
void LobbyManager::RouteToRoom(int playerId, const game::GameMessage& msg,
    ...
    else if (msg.has_reload_event()) {
        // Relay reload event to all players in room
...
void LobbyManager::HandleLoginRequest(const game::GameMessage& msg,
    ...
    // Check if account is already logged in; reject unless old session is stale
    {
        ...
        for (auto& pa : playerAccounts) {
            if (pa.second == account) {
                ...
                if (oldIt != clients.end() && oldIt->second != sender) {
                    // If old session is still active (heartbeat within 5s), reject
                    auto lsIt = lastSeen.find(pa.first);
                    if (lsIt != lastSeen.end() && (now - lsIt->second) < std::chrono::seconds(5)) {
                        resp->set_success(false);
                    ...
                    staleIds.push_back(pa.first);
                }
            }
        }
        for (int oldId : staleIds) {
            std::cout << "[Lobby] Player " << oldId
                      << " kicked (account '" << account << "' re-logged in, old session stale)" << std::endl;
            LeaveRoom(oldId);
            clients.erase(oldId);
            lastSeen.erase(oldId);
            playerAccounts.erase(oldId);
        }
    }
